...
def read_audio(filename, sample_rate=SAMPLE_RATE):
    ...
    if len(audio) < (end_frame - start_frame): #如果音频不够长,补齐, 长的音频就保留全部
        au = [0] * (end_frame - start_frame) #补齐空白.
        for i in range(len(audio)):
            au[i] = audio[i] # 贴过来原始的语音
        audio = np.array(au)
    return audio
...
def extract_features(signal=np.random.uniform(size=48000), target_sample_rate=SAMPLE_RATE):
    filter_banks, energies = fbank(signal, samplerate=target_sample_rate, nfilt=64, winlen=0.025)   #filter_bank (num_frames , 64),energies (num_frames ,)  # 返回第一个变量是特征,第二个是能量.
