...
def convolutional_model(input_shape=(NUM_FRAMES,64, 1),    #input_shape(32,32,3)
    ...
    def cnn_component(inp): # 网络结构就是resnet的cnn.
    ...
    x = cnn_component(inputs)  # .shape = (BATCH_SIZE , num_frames/16, 64/16, 512) #====输出shape特征变成512了.
    #x = Reshape((-1,2048))(x)
    x = Lambda(lambda y: K.reshape(y, (-1, math.ceil(num_frames/16), 2048)), name='reshape')(x) #改变shape
    x = Lambda(lambda y: K.mean(y, axis=1), name='average')(x)  #shape = (BATCH_SIZE, 512) # 消除时间维度,平均化即可
