...
def clipped_audio(x, num_frames=c.NUM_FRAMES): # 随机在数据上抽取160帧.
...
class MiniBatch:
    def __init__(self, libri, batch_size, unique_speakers=None):    #libri['filename']，libri['chapter_id']，libri['speaker_id']，libri['dataset_id']
        ...
        for ii in range(num_triplets):
            two_different_speakers = np.random.choice(unique_speakers, size=2, replace=False) # 选2个不同的人,分别作为正负样本
            ...
            negative_df = libri[libri['speaker_id'] == negative_speaker].sample(n=1) #抽取一个负样本.
