...
def create_data_producer(unique_speakers, spk_utt_dict,candidates=c.CANDIDATES_PER_BATCH):
    ...
    producer.setDaemon(True) # thread.setDaemon（）设置为True, 则主线程执行完毕后会将子线程回收掉,
    producer.start()
...
def addstack(unique_speakers, spk_utt_dict,candidates=c.CANDIDATES_PER_BATCH):
    ...
    while True:
        ...
        stack.append((feature, labels)) # 数据写入全局变量stack中
...
def best_batch(model, batch_size=c.BATCH_SIZE,candidates=c.CANDIDATES_PER_BATCH):
    ...
    features,labels = getbatch() # 一个batch 有640个数据
    ...
    speakers_embeds = hist_embeds[inds_set] # speakers_embeds 是我们抽取部分的特征,hist_embeds是全部特征
    ...
    for ii in range(int(batch_size/2)):   #每一轮找出两对triplet pairs
        while True:
            speaker = anh_speakers[ii] #选一个用户编号.
            inds = anchs_index_dict[speaker] # 获取这个用户的全部语音文件的索引
            np.random.shuffle(inds)
            anchor_index = inds[0] #随便抽一个语音.
            pinds = []
            for jj in range(1,len(inds)):
                if (hist_features[anchor_index] == hist_features[inds[jj]]).all():
                    continue
                pinds.append(inds[jj])  # 找到同一个人的不同语音放入pinds里面

            if len(pinds) >= 1: # 如果pinds==[]说明这个人只有一条语音,这是没法做tripleloss的.所以继续循环找其他人.
                break

        sap = sims[ii][pinds]# 得到a 和p 的相似度. similarity 所以缩写是sap, 这里面需要理解到 ii 和pinds都是一个人的语音. 不懂的自己体会一下上面的for循环即可.
        min_saps = heapq.nsmallest(2, sap) #找到相似度最小的2个.
...
#======到此就已经做好了数据!!!!!!!!!
    ...
    return batch, np.array(labs) # 总结:所以我们最后得到了triple 数据和他们对应的label. 都是对应96个元素. 每32个是一组.组内的anchor是一个用户.p是一个跟anchor同用户,n是另外一个用户
